...
def dataset_split(train_images, train_labels, args,  noise_rate=0.5, noise_type='sym', random_seed=1, num_classes=10, include_noise=False):
    ...
    if noise_type == 'instance':
        ...
        noisy_labels, real_noise_rate, transition_matrix = noisify_multiclass_symmetric(clean_train_labels, noise_rate=noise_rate, random_state=random_seed, nb_classes=num_classes)
...
def noisify_multiclass_symmetric(y_train, noise_rate, random_state=None, nb_classes=10):
    """
       y_train=clean_train_labels, noise=noise_rate
    """
    noise_label = []
    idx = list(range(50000))
    random.shuffle(idx)
    num_noise = int(50000*noise_rate)
    ...
    for i in range(50000):
        if i in noise_idx:
            ...
            noise_label.append(y_train[i][0])

    noise_label = np.array(noise_label).reshape(-1, 1)
    return noise_label, None, None
